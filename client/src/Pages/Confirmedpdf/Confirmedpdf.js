        ...
        axios.get(`/isfine/${id}`)
            .then((res) => {
                if (res.data) {
                    ...
                    const onsite = res.data.Onsite.map(item => item.user);
                    const usersWithFineArray = res.data.usersWithFine.map(item => item.user);
                    const otGiven = res.data.OtGiven.map(item => item.user);
                    const teGiven = res.data.TeGiven.map(item => item.user);
                    setOnSite(onsite);
                    setIsFine(usersWithFineArray);
                    setOtGiven(otGiven);
                    setTeGiven(teGiven);
                } else {
                    setIsFine([]);
                                ...
                                <tbody className="text-center">
                                    ...
                                    {values.map((user, index) => (
                                        <tr key={user._id}>
                                            ...
                                            <td>{user.role}</td>
                                            <td>
                                                {tegiven && tegiven.includes(user._id) ? (

                                                    <Link to={`/viewte/${user._id}?eventId=${id}`} className="btn btn-warning">
                                                        View Te
                                                    </Link>

                                                ) : (
                                                    <Link to={`/te/${user._id}?eventId=${id}`} className="btn btn-primary">
                                                        TE</Link>
                                                )}

                                                &nbsp;&nbsp;
                                                {otgiven && otgiven.includes(user._id) ? (

                                                    <Link to={`/viewot/${user._id}?eventId=${id}`} className="btn btn-warning">
                                                        View Ot
                                                    </Link>
                                                ...
                                                {isFine && isFine.includes(user._id) ? (
                                                    <Link to={`/viewfine/${user._id}?eventId=${id}`} className="btn btn-warning">
                                                        View Fine
                                                    </Link>
                                                ) : (
                                                ...
                                                &nbsp;&nbsp;
                                                {onSite && onSite.includes(user._id) ? (

                                                    <Link to={`/viewsalary/${user._id}?eventId=${id}`} className="btn btn-warning">
                                                        View Salary
                                                    </Link>

                                                ) : (
                                                    <button className="btn btn-success" onClick={() => handleOnsite(user._id)}>
                                                        Onsite
                                                    </button>
                                                )}
